/*
 * MATLAB Compiler: 4.2 (R14SP2)
 * Date: Mon Oct 27 10:38:38 2008
 * Arguments: "-B" "macro_default" "-m" "-W" "main" "-T" "link:exe"
 * "guiEDISA.m" 
 */


#ifdef __cplusplus
extern "C" {
#endif
const unsigned char __MCC_guiEDISA_public_data[] = {'3', '0', '8', '1', '9',
                                                    'D', '3', '0', '0', 'D',
                                                    '0', '6', '0', '9', '2',
                                                    'A', '8', '6', '4', '8',
                                                    '8', '6', 'F', '7', '0',
                                                    'D', '0', '1', '0', '1',
                                                    '0', '1', '0', '5', '0',
                                                    '0', '0', '3', '8', '1',
                                                    '8', 'B', '0', '0', '3',
                                                    '0', '8', '1', '8', '7',
                                                    '0', '2', '8', '1', '8',
                                                    '1', '0', '0', 'C', '4',
                                                    '9', 'C', 'A', 'C', '3',
                                                    '4', 'E', 'D', '1', '3',
                                                    'A', '5', '2', '0', '6',
                                                    '5', '8', 'F', '6', 'F',
                                                    '8', 'E', '0', '1', '3',
                                                    '8', 'C', '4', '3', '1',
                                                    '5', 'B', '4', '3', '1',
                                                    '5', '2', '7', '7', 'E',
                                                    'D', '3', 'F', '7', 'D',
                                                    'A', 'E', '5', '3', '0',
                                                    '9', '9', 'D', 'B', '0',
                                                    '8', 'E', 'E', '5', '8',
                                                    '9', 'F', '8', '0', '4',
                                                    'D', '4', 'B', '9', '8',
                                                    '1', '3', '2', '6', 'A',
                                                    '5', '2', 'C', 'C', 'E',
                                                    '4', '3', '8', '2', 'E',
                                                    '9', 'F', '2', 'B', '4',
                                                    'D', '0', '8', '5', 'E',
                                                    'B', '9', '5', '0', 'C',
                                                    '7', 'A', 'B', '1', '2',
                                                    'E', 'D', 'E', '2', 'D',
                                                    '4', '1', '2', '9', '7',
                                                    '8', '2', '0', 'E', '6',
                                                    '3', '7', '7', 'A', '5',
                                                    'F', 'E', 'B', '5', '6',
                                                    '8', '9', 'D', '4', 'E',
                                                    '6', '0', '3', '2', 'F',
                                                    '6', '0', 'C', '4', '3',
                                                    '0', '7', '4', 'A', '0',
                                                    '4', 'C', '2', '6', 'A',
                                                    'B', '7', '2', 'F', '5',
                                                    '4', 'B', '5', '1', 'B',
                                                    'B', '4', '6', '0', '5',
                                                    '7', '8', '7', '8', '5',
                                                    'B', '1', '9', '9', '0',
                                                    '1', '4', '3', '1', '4',
                                                    'A', '6', '5', 'F', '0',
                                                    '9', '0', 'B', '6', '1',
                                                    'F', 'C', '2', '0', '1',
                                                    '6', '9', '4', '5', '3',
                                                    'B', '5', '8', 'F', 'C',
                                                    '8', 'B', 'A', '4', '3',
                                                    'E', '6', '7', '7', '6',
                                                    'E', 'B', '7', 'E', 'C',
                                                    'D', '3', '1', '7', '8',
                                                    'B', '5', '6', 'A', 'B',
                                                    '0', 'F', 'A', '0', '6',
                                                    'D', 'D', '6', '4', '9',
                                                    '6', '7', 'C', 'B', '1',
                                                    '4', '9', 'E', '5', '0',
                                                    '2', '0', '1', '1', '1'
                                                    , '\0'};

const char *__MCC_guiEDISA_name_data = "guiEDISA";

const char *__MCC_guiEDISA_root_data = "";

const unsigned char __MCC_guiEDISA_session_data[] = {'3', 'C', '6', '6', 'D',
                                                     '0', '1', 'C', '6', 'B',
                                                     '3', '9', 'F', '5', '3',
                                                     'C', '9', '2', '0', '5',
                                                     '2', 'B', 'C', '7', '1',
                                                     'F', '6', '0', '0', 'A',
                                                     '5', '7', '1', '8', 'C',
                                                     '8', '4', 'B', 'F', '9',
                                                     '5', 'D', '8', 'E', 'C',
                                                     'D', '3', '6', '6', 'B',
                                                     '2', '4', '1', '6', '9',
                                                     'F', '5', 'E', '0', '9',
                                                     'B', '6', 'A', 'C', 'F',
                                                     '5', '8', '9', '4', 'E',
                                                     '9', '9', '4', '8', '7',
                                                     '8', '4', '9', '4', '7',
                                                     '4', '2', '1', '5', '7',
                                                     'C', '4', '3', '4', '6',
                                                     '2', 'E', '5', '5', '0',
                                                     '9', '2', '0', '3', 'B',
                                                     'A', 'E', 'D', '0', '3',
                                                     '5', 'D', '0', '0', '1',
                                                     '2', 'C', 'A', '8', '1',
                                                     '5', '5', '2', '4', '6',
                                                     'B', '2', 'A', 'D', '5',
                                                     'C', '9', '8', 'B', '4',
                                                     '9', 'C', 'D', 'E', '0',
                                                     '4', '2', '4', 'B', '0',
                                                     '7', 'B', '7', '7', '6',
                                                     'D', '5', 'D', '9', '6',
                                                     '9', '1', '8', '0', 'A',
                                                     'D', 'E', '2', '5', '6',
                                                     '1', '0', '4', 'D', 'F',
                                                     'A', 'F', '9', 'B', '5',
                                                     'E', 'F', '7', '0', '2',
                                                     '1', 'A', 'C', '1', '3',
                                                     '6', 'F', '3', '2', 'A',
                                                     '4', '0', '5', 'A', '6',
                                                     'E', 'E', '8', '1', 'C',
                                                     '5', '3', '1', '7', '2',
                                                     'C', '9', '3', '9', '5',
                                                     '1', '5', '4', '3', '3',
                                                     '0', 'A', 'E', '6', 'B',
                                                     '3', '7', 'C', '4', '3',
                                                     '0', '1', '0', '5', '0',
                                                     'A', '9', '9', '7', 'C',
                                                     '6', '9', '3', '2', '7',
                                                     'D', '6', '6', '5', 'D',
                                                     'A', 'A', '9', '0', '3',
                                                     '4', '1', '9', '1', '8',
                                                     'C', '3', 'C', '5', '2',
                                                     'E', '\0'};

const char *__MCC_guiEDISA_matlabpath_data[] = {"guiEDISA/",
                                                "toolbox/compiler/deploy/",
                                                "Programme/matlab/SBADDON/",
                                                "Programme/matlab/SBADDON/examples/",
                                                "$TOOLBOXMATLABDIR/general/",
                                                "$TOOLBOXMATLABDIR/ops/",
                                                "$TOOLBOXMATLABDIR/lang/",
                                                "$TOOLBOXMATLABDIR/elmat/",
                                                "$TOOLBOXMATLABDIR/elfun/",
                                                "$TOOLBOXMATLABDIR/specfun/",
                                                "$TOOLBOXMATLABDIR/matfun/",
                                                "$TOOLBOXMATLABDIR/datafun/",
                                                "$TOOLBOXMATLABDIR/polyfun/",
                                                "$TOOLBOXMATLABDIR/funfun/",
                                                "$TOOLBOXMATLABDIR/sparfun/",
                                                "$TOOLBOXMATLABDIR/scribe/",
                                                "$TOOLBOXMATLABDIR/graph2d/",
                                                "$TOOLBOXMATLABDIR/graph3d/",
                                                "$TOOLBOXMATLABDIR/specgraph/",
                                                "$TOOLBOXMATLABDIR/graphics/",
                                                "$TOOLBOXMATLABDIR/uitools/",
                                                "$TOOLBOXMATLABDIR/strfun/",
                                                "$TOOLBOXMATLABDIR/imagesci/",
                                                "$TOOLBOXMATLABDIR/iofun/",
                                                "$TOOLBOXMATLABDIR/audiovideo/",
                                                "$TOOLBOXMATLABDIR/timefun/",
                                                "$TOOLBOXMATLABDIR/datatypes/",
                                                "$TOOLBOXMATLABDIR/verctrl/",
                                                "$TOOLBOXMATLABDIR/codetools/",
                                                "$TOOLBOXMATLABDIR/helptools/",
                                                "$TOOLBOXMATLABDIR/winfun/",
                                                "$TOOLBOXMATLABDIR/demos/",
                                                "$TOOLBOXMATLABDIR/timeseries/",
                                                "$TOOLBOXMATLABDIR/hds/",
                                                "toolbox/local/",
                                                "toolbox/compiler/",
                                                "toolbox/optim/",
                                                "toolbox/stats/"};
const int __MCC_guiEDISA_matlabpath_data_count = 38;

const char *__MCC_guiEDISA_classpath_data[] = { "" };
const int __MCC_guiEDISA_classpath_data_count = 0;

const char *__MCC_guiEDISA_lib_path_data[] = { "" };
const int __MCC_guiEDISA_lib_path_data_count = 0;

const char *__MCC_guiEDISA_mcr_application_options[] = { "" };
const int __MCC_guiEDISA_mcr_application_option_count = 0;
const char *__MCC_guiEDISA_mcr_runtime_options[] = { "" };
const int __MCC_guiEDISA_mcr_runtime_option_count = 0;
#ifdef __cplusplus
}
#endif


